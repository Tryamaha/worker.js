export default {
  async fetch(request, env) {
    try {
      ...
      if (path === "/health") return json({ ok: true, app: "Spam Kovucu AppStore Final", status: "healthy" });
...
async function analyze(url, env) {
  ...
  await env.DB.prepare("INSERT INTO memory(phone,score,created_at) VALUES(?,?,?)")
    .bind(phone, score, now()).run();
...
<ul>${s.top.map(x => `<li>${x.phone} — ${x.c} sorgu</li>`).join("") || "<li>Kayıt yok</li>"}</ul>
</div>

<div class="card">
<h2>🚨 Son İhbarlar</h2>
<ul>${s.lastReports.map(x => `<li>${x.phone} — ${x.type} — ${x.note || ""}</li>`).join("") || "<li>İhbar yok</li>"}</ul>
</div>

<div class="card">
<h2>⛔ Kara Liste</h2>
<ul>${s.lastBlack.map(x => `<li>${x.phone} — ${x.reason}</li>`).join("") || "<li>Kayıt yok</li>"}</ul>
</div>
...
a{color:#7dd3fc}.muted{color:#cbd5e1;line-height:1.5}li{margin:9px 0;color:#dbeafe}
</style>
</head>
<body>
...
<div class="sub">AppStore Final • AI + D1 hafıza + ihbar + kara liste + Google OSINT</div>
...
localStorage.setItem('lastnum',n);
...
'<div class="glass"><h2>🌍 Google Açık Web Araştırması</h2>'+d.webResults.map(x=>'<p><a target="_blank" href="'+x.link+'">'+x.title+'</a></p>').join('')+'</div>'+
